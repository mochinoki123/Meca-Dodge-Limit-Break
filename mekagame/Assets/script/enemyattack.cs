...
public class enemyattack : MonoBehaviour
{
  ...
  //[SerializeField] int attackpointz3;//もしも用　使ってない
    ...
    [SerializeField] float maxLength = -70f;   // 最終的な長さ-70
    [SerializeField] float extendSpeed = 100;  // 伸びるスピード80
    ...
    IEnumerator AttackLoop2Coroutine()
    {
        while (enemyhpscripts.EnemyHP > 500)//敵のHP条件
        {
            ...
            yield return new WaitForSeconds(1.7f);//2秒ごとにループする
        }
    ...
    IEnumerator AttackLoop3Coroutine()
    {
        while (enemyhpscripts.EnemyHP > 250)//敵のHP条件
        {
            ...
            yield return new WaitForSeconds(1.3f);//2秒ごとにループする
        }
    ...
    void Attack4b()
    {
        ...
        {
            GameObject Attack4bpoint1 = Instantiate(bpoint, new Vector3(ap, 0, ap + 10 * i), Quaternion.identity);//北
            GameObject Attack4bpoint2 = Instantiate(bpoint, new Vector3(ap + 10 * i, 0, ap), Quaternion.identity);//東
            GameObject Attack4bpoint3 = Instantiate(bpoint, new Vector3(ap, 0, ap - 10 * i), Quaternion.identity);//南
            GameObject Attack4bpoint4 = Instantiate(bpoint, new Vector3(ap - 10 * i, 0, ap), Quaternion.identity);//西
